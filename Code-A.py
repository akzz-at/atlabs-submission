#Code which keeps the original time stamps the same and approximates the words with no start_time and end_time
...
def adjust_time_with_pointers(data):
    pointer1 = 0
    pointer2 = 0
    n = len(data)

    # Handling the case where the first k words have None start and end times
    if data[0][1] is None or data[0][2] is None:
        data[0] = (data[0][0], 0.0, 0.1)

    while pointer1 < n and pointer2 < n:
        # pointer1 advances until it finds a None
        while pointer1 < n and data[pointer1][1] is not None and data[pointer1][2] is not None:
            pointer1 += 1
        
        pointer2 = pointer1

        # pointer2 advances until it finds a non-None
        while pointer2 < n and (data[pointer2][1] is None or data[pointer2][2] is None):
            pointer2 += 1

        # If pointers are not equal and pointer2 is not beyond the list's limits
        if pointer1 != pointer2 and pointer2 < n and (pointer1 > 0 or data[0][1] == 0.0):
            prev_end = data[pointer1-1][2] if pointer1 > 0 else 0.0
            total_gap = pointer2 - pointer1
            time_diff = round((data[pointer2][1] - prev_end) / total_gap, 1)

            # Update the times for the None values
            for i in range(pointer1, pointer2):
                start_time = round(prev_end, 1)
                end_time = round(start_time + time_diff, 1)
                prev_end = end_time
                data[i] = (data[i][0], start_time, end_time)
